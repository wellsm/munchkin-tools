...
import { DEFAULT_MAX_LEVEL, DEFAULT_MAX_PLAYERS } from "./constants";
...
describe("munchkin store", () => {
  ...
  it("setPartyModifier / setMonsterLevel / setMonsterModifier update values", () => {
    const { setPartyModifier, setMonsterLevel, setMonsterModifier } =
      useMunchkinStore.getState();
    setPartyModifier(3);
    setMonsterLevel(7);
  ...
  it("resetCombat zeroes combat state without touching players", () => {
    addHero("Ana");
    const id = useMunchkinStore.getState().players[0].id;
    const { setMainCombatant, setPartyModifier, setMonsterLevel, resetCombat } =
      useMunchkinStore.getState();
    setMainCombatant(id);
    setPartyModifier(5);
    setMonsterLevel(8);
